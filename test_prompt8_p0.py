...
class TestPrompt8bP0(unittest.TestCase):
    def setUp(self):
        ...
        backups = sorted(Path(BASE_DIR / "backups").glob("money_tracks_pre_ledger_repair_*.db"), key=lambda p: p.stat().st_mtime, reverse=True)
        if not backups:
            backups = sorted(Path(BASE_DIR / "backups").glob("money_tracks_*.db"), key=lambda p: p.stat().st_mtime, reverse=False)
