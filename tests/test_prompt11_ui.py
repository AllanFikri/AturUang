...
class TestPrompt11UI(unittest.TestCase):
    def setUp(self):
        ...
        self.db_path = self.repo_dir / "runtime" / "money_tracks.db"
        if not self.db_path.exists():
            self.db_path = self.repo_dir / "money_tracks.db"
        self.prod_hash_before = hashlib.sha256(self.db_path.read_bytes()).hexdigest() if self.db_path.exists() else None
        self.index_html_path = self.repo_dir / "aturuang" / "web" / "index.html"
        self.app_js_path = self.repo_dir / "aturuang" / "web" / "js" / "app.js"
        self.core_js_path = self.repo_dir / "aturuang" / "web" / "js" / "core.js"
        self.styles_css_path = self.repo_dir / "aturuang" / "web" / "css" / "styles.css"
        ...
        shutil.copyfile(self.db_path, self.test_db)
    ...
    def test_09_production_db_hash_unmutated(self):
        ...
        prod_db = self.db_path
        if self.prod_hash_before is not None and prod_db.exists():
