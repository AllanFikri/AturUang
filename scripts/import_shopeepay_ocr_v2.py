...
RE_FAILED = re.compile(r"\b(Failed|Gagal|Reward Claimed)\b", re.IGNORECASE)
...
def parse(text, period):
    ...
    rows = []; i = 0
    while i < len(lines):
        m = RE_LINE_AMOUNT.match(lines[i])
        if not m:
            i += 1; continue
        lt, sign, amt_str = m.group(1), m.group(2), m.group(3)
...
print()
print(f"TOTAL: {len(all_rows)} transaksi")
...
if APPLY:
    ...
    bak = Path(f"runtime/money_tracks.db.bak-sp-ocr-{ts}")
    ...
    for r in all_rows:
        ...
        # Idempotency: cek apakah sudah ada (source_refs + date + amount + description)
        existing = cur.execute("""
            SELECT id FROM transactions
            WHERE date = ? AND amount = ? AND source_refs LIKE 'shopeepay_ocr%'
              AND substr(description,1,40) LIKE ?
              AND is_deleted = 0
        """, (r["date"], r["amount"], f"[ShopeePay OCR] {r['line_type']}%")).fetchone()
        ...
        """, (
            ...
            1, f"ShopeePay evidence (V3-STEP-100E)",
            "legacy", "", import_id,
            f"[V3-STEP-100E: ShopeePay OCR {r['source_file']}; {r['line_type']}]",
            0, 0,
